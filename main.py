import flet as ft
import pandas as pd
from element_string_lists import element_z_to_name, element_z_to_symbol, element_z_to_symbol_z, element_symbol_to_name
...
def get_available_line_energies(atomic_df:pd.DataFrame, z:int) -> list[tuple[str,float]]:
	"""
	...
	"""
	line_names = ["Ka1", "Ka2", "Kb1", "Kb2", "Kb3", "La1", "La2", "Lb1", "Lb2", "Lb3", "Lb4", "Lg1", "Lg2", "Lg3", "Ll"]
...
def main(page: ft.Page):
	...
	def dropdown_elementofinterest_changed(e) -> None:
		...
		for _line, _energy in get_available_line_energies(atomic_df, int(dropdown_elementofinterest.value)):
			dropdown_lineofinterest.options.append(ft.dropdown.Option(key = _energy, text = f"{element_z_to_symbol(int(dropdown_elementofinterest.value))} {_line}\t({_energy} keV)"))
		page.update()
	...
	def dropdown_lineofinterest_changed(e) -> None:
		...
		result = get_fluorescence_depth_mm(matrix_element=matrix_ele_sym, fluorescence_energy_ev=line_energy_ev, detectable_photon_fraction=pctg_counts_detectable)
		print(result)
	...
	dropdown_elementofinterest = ft.Dropdown(label="Element of Interest", on_change=dropdown_elementofinterest_changed, helper_text="The Element you're trying to detect")
	dropdown_matrixelement = ft.Dropdown(label="Matrix Element", helper_text="The Element covering the Element-of-interest")
	dropdown_lineofinterest = ft.Dropdown(label="Spectral Line", helper_text="Which line are you using to analyse your element of interest?", on_change=dropdown_lineofinterest_changed)
	for ele_z, ele_sym, ele_name in df_to_z_sym_name_tuples(atomic_df):
		...
		dropdown_elementofinterest.options.append(ft.dropdown.Option(key = ele_z, text = f"{ele_z}\t{ele_sym}\t{ele_name}"))
		dropdown_matrixelement.options.append(ft.dropdown.Option(key = ele_z, text = f"{ele_z}\t{ele_sym}\t{ele_name}"))
	...
	container_root = ft.Container(
		content=ft.Column(controls=[dropdown_elementofinterest, dropdown_matrixelement, dropdown_lineofinterest, text_spiel, text_result]),
